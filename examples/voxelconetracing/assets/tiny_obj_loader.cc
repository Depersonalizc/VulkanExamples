...
namespace tinyobj {
    ...
    std::string LoadMtl(std::map<std::string, int> &material_map,
...
#if defined(__ANDROID__)
        char* buf = static_cast<char*>( malloc(maxchars * sizeof(char)) );
        size_t n = 0;
        ...
        while (inStream.peek() != -1) {
            ...
            if ((0 == strncmp(token, "newmtl", 6)) && isSpace((token[6]))) {
                ...
                token += 7;
#ifdef _MSC_VER
                sscanf_s(token, "%s", namebuf, (unsigned int)sizeof(char) * 4096);
#else
    ...
    std::string MaterialFileReader::operator()(const std::string &matId,
        ...
        if (!matFile) {
            return "";  // Materials stored in image files
        }
        return LoadMtl(matMap, materials, matFile);
#else
    ...
    std::string LoadObj(std::vector<shape_t> &shapes,
        ...
        return LoadObj(shapes, materials, file, matFileReader);
#else
