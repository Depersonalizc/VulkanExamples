...
VulkanApp::VulkanApp()
#if INBASE
	: WIDTH(1080), HEIGHT(600), physicalDevice(VK_NULL_HANDLE), LayerCount(1)
#endif
{
...
void VulkanApp::LoadGlobalMaterials(PostProcess *vxgiPostProcess)
{
...
void VulkanApp::LoadObjects()
{
	...
	Cerberus->init(device, physicalDevice, deferredCommandPool, objectDrawQueue, "objects/Cerberus.obj", 0, false);
...
void VulkanApp::initVulkan() 
{
	...
	*/
	...
	//Global Materials
	LoadGlobalMaterials(VXGIPostProcess);
	...
	{
		objectManager[i]->createShadowMaterial(
			standardShadow.commandPool, standardShadow.queue, standardShadow.renderPass,
			glm::vec2(static_cast<float>(standardShadow.Extent2D.width), static_cast<float>(standardShadow.Extent2D.height)),
...
//	}
//
#if 0  // TODO: indices is the same as VulkanExample::vulkanDevice::queueFamilyIndices
...
{
	QueueFamilyIndices queueFamilyIndices = findQueueFamilies(physicalDevice, surface);  // TODO: Use VulkanExample::vulkanDevice::queueFamilyIndices
...
void VulkanApp::createFrameBufferCommandBuffers()  // VulkanExample::buildCommandBuffers() override
{
#if 1
	//Left and Main
...
void VulkanApp::cleanUpSwapChainAtResize()
{
	// delete depth
	...
	{
		PostProcess* thisPostProcess = postProcessStages[i];
		thisPostProcess->cleanUp();  // image, imageview, imagemem
	...
	{
		PostProcess* thisPostProcess = postProcessStages[i];
		thisPostProcess->material->cleanPipeline();  // destroy pipeline and pipeline layout
